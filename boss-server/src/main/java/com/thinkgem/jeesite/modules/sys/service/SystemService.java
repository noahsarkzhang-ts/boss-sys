...
//		user.setPage(page);
		...
		page.setList(userDao.findList(params));
		return page;
	}
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public void updateUserLoginInfo(User user) {
		...
		// 更新本次登录信息
		user.setLoginIp(Servlets.getRemoteAddr(Servlets.getRequest()));
	...
	public void saveRole(Role role) {
		...
		if (role.getId() == null){
			role.preInsert(callback);
			roleDao.insert(role);
		}else{
			role.preUpdate(callback);
			roleDao.update(role);
		}
		// 更新角色与菜单关联
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public void saveMenu(Menu menu) {
		...
		// 保存或更新实体
		if (menu.getId() == null){
			menu.preInsert(callback);
			menuDao.insert(menu);
		}else{
			menu.preUpdate(callback);
			menuDao.update(menu);
		}
