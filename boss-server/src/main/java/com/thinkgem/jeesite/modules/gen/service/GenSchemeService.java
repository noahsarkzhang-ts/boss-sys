...
//	private GenTemplateDao genTemplateDao;
	...
	public Page<GenScheme> find(Page<GenScheme> page, GenScheme genScheme) {
		...
		params.put("entity", genScheme);
		params.put("page", page);
		
		page.setList(genSchemeDao.findList(params));
		return page;
	}
	...
	public String save(GenScheme genScheme) {
		...
		if (genScheme.getId() == null){
			genScheme.preInsert(callback);
			genSchemeDao.insert(genScheme);
		}else{
			genScheme.preUpdate(callback);
	...
	private String generateCode(GenScheme genScheme){
		...
		Map<String,Object> params = Maps.newHashMap();
		params.put("entity", new GenTableColumn(new GenTable(genTable.getId())));
		...
		if (childTableTemplateList.size() > 0){
			...
			params.put("entity", parentTable);
			
			genTable.setChildList(genTableDao.findList(param));
		}
		...
		for (GenTable childTable : genTable.getChildList()){
			...
			Map<String,Object> param = Maps.newHashMap();
			params.put("entity", new GenTableColumn(new GenTable(childTable.getId())));
			
			childTable.setColumnList(genTableColumnDao.findList(param));
